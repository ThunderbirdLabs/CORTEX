...
def batch_detect_urgency_task(
    ...
    """
    try:
        logger.info(f"🔄 Starting batch urgency detection for tenant {tenant_id} (limit: {limit})")
        ...
        logger.error(f"❌ Batch urgency detection failed for tenant {tenant_id}: {e}", exc_info=True)
        raise
