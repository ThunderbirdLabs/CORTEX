...
def generate_daily_intelligence_task(tenant_id: str, target_date: Optional[str] = None):
    """
    Generate daily intelligence summary for a specific tenant and date.

    Args:
        tenant_id: Tenant ID to generate intelligence for
    ...
    logger.info(f"🌙 Starting daily intelligence generation for {tenant_id} on {date_obj}")

    try:
        # Run async calculation
        asyncio.run(_generate_daily_intelligence_async(tenant_id, date_obj))
        logger.info(f"✅ Daily intelligence completed for {tenant_id} on {date_obj}")

    except Exception as e:
        logger.error(f"❌ Daily intelligence failed for {tenant_id} on {date_obj}: {e}", exc_info=True)
        raise


async def _generate_daily_intelligence_async(tenant_id: str, target_date: date):
    ...
    try:
        ...
        result = supabase.table("daily_intelligence")\
            .upsert(metrics, on_conflict="tenant_id,date")\
            .execute()
...
def generate_weekly_intelligence_task(tenant_id: str, week_start: Optional[str] = None):
    """
    ...
    logger.info(f"📅 Starting weekly intelligence generation for {tenant_id}, week of {date_obj}")

    try:
        # Run async calculation
        asyncio.run(_generate_weekly_intelligence_async(tenant_id, date_obj))
        logger.info(f"✅ Weekly intelligence completed for {tenant_id}, week of {date_obj}")

    except Exception as e:
        logger.error(f"❌ Weekly intelligence failed for {tenant_id}, week of {date_obj}: {e}", exc_info=True)
        raise


async def _generate_weekly_intelligence_async(tenant_id: str, week_start: date):
    ...
    try:
        ...
        result = supabase.table("weekly_intelligence")\
            .upsert(metrics, on_conflict="tenant_id,week_start")\
            .execute()
...
def generate_monthly_intelligence_task(tenant_id: str, month: Optional[str] = None):
    """
    ...
    logger.info(f"📊 Starting monthly intelligence generation for {tenant_id}, month {date_obj.strftime('%B %Y')}")

    try:
        # Run async calculation
        asyncio.run(_generate_monthly_intelligence_async(tenant_id, date_obj))
        logger.info(f"✅ Monthly intelligence completed for {tenant_id}, month {date_obj.strftime('%B %Y')}")

    except Exception as e:
        logger.error(f"❌ Monthly intelligence failed for {tenant_id}, month {date_obj.strftime('%B %Y')}: {e}", exc_info=True)
        raise


async def _generate_monthly_intelligence_async(tenant_id: str, month: date):
    ...
    try:
        ...
        result = supabase.table("monthly_intelligence")\
            .upsert(metrics, on_conflict="tenant_id,month")\
            .execute()
...
def generate_intelligence_for_all_tenants(period: str, target_date: Optional[str] = None):
    ...
    try:
        ...
        tenant_ids = [row['tenant_id'] for row in result.data]
        logger.info(f"Found {len(tenant_ids)} tenants to process")

        # Enqueue individual tasks
        for tenant_id in tenant_ids:
            if period == "daily":
                generate_daily_intelligence_task.send(tenant_id, target_date)
            elif period == "weekly":
                generate_weekly_intelligence_task.send(tenant_id, target_date)
            elif period == "monthly":
                generate_monthly_intelligence_task.send(tenant_id, target_date)
            else:
                logger.error(f"Unknown period: {period}")
                continue

        logger.info(f"✅ Enqueued {len(tenant_ids)} {period} intelligence tasks")
