...
def normalize_quickbooks_invoice(invoice: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
    """
...
def normalize_quickbooks_bill(bill: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
...
def normalize_quickbooks_payment(payment: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
...
def normalize_quickbooks_customer(customer: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
...
async def run_quickbooks_sync(
    ...
    Returns:
        Sync statistics
    """
    logger.info(f"🚀 Starting QuickBooks sync for tenant {tenant_id}")
    ...
    try:
        ...
        if not connection_id:
            error_msg = f"No QuickBooks connection found for tenant {tenant_id}"
        ...
        try:
            ...
            for invoice in invoices:
                try:
                    ...
                    normalized = normalize_quickbooks_invoice(invoice, tenant_id)
        ...
        try:
            ...
            for bill in bills:
                try:
                    normalized = normalize_quickbooks_bill(bill, tenant_id)
                    await ingest_document_universal(
                        supabase=supabase,
                        cortex_pipeline=cortex_pipeline,
                        **normalized
        ...
        try:
            ...
            for payment in payments:
                try:
                    normalized = normalize_quickbooks_payment(payment, tenant_id)
                    await ingest_document_universal(
                        supabase=supabase,
                        cortex_pipeline=cortex_pipeline,
                        **normalized
        ...
        try:
            ...
            for customer in customers:
                try:
                    normalized = normalize_quickbooks_customer(customer, tenant_id)
                    await ingest_document_universal(
                        supabase=supabase,
                        cortex_pipeline=cortex_pipeline,
                        **normalized
        ...
        # ========================================================================
        logger.info("=" * 80)
        logger.info(f"✅ QuickBooks sync complete for tenant {tenant_id}")
        ...
        logger.error(f"❌ QuickBooks sync failed for tenant {tenant_id}: {e}")
        return {
            "status": "error",
            "tenant_id": tenant_id,
            "records_synced": records_synced,
