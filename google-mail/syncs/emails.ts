...
// MODIFIED: Changed from 365 days to 7 days (1 week)
const DEFAULT_BACKFILL_MS = 7 * 24 * 60 * 60 * 1000;

const sync = createSync({
    description:
        'Fetches a list of emails from gmail. Goes back default to 1 week (7 days)\nbut metadata can be set using the `backfillPeriodMs` property\nto change the lookback. The property should be set in milliseconds.',
    version: '2.0.2',
...
function mapEmail(messageDetail: Schema$Message, headers: Record<string, any>): GmailEmail {
    ...
    return {
        ...
        date: new Date(parseInt(messageDetail.internalDate || '0')).toISOString(),
